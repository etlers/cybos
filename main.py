...
# 계좌번호
account_no = deal_info["account_no"]
# 최초 지갑
...
def get_possesion():
    try:
        ...
        # 예수금
        in_possesion = int(df['예수금'][0])
        print(format(in_possesion, ','))
    except:
        in_possesion = 0
...
def get_now_price():
    ...
    if (exFlag == ord('0')):
        print("장 구분값: 동시호가와 장중 이외의 시간", time, cprice, open)
    elif (exFlag == ord('1')) :
        print("장 구분값: 동시호가 시간", time, cprice, open)
    elif (exFlag == ord('2')):
        print("장 구분값: 장중 또는 장종료", time, cprice, open)
    # 가격정보 저장
    list_temp = []        
...
# 매수
def buy_n_sell(order_div, order_qty):
    try:
        # 시장가주문 매수, 매도
        kiwoom.SendOrder(
            "시장가매수", 
            "0101", 
            dict_account["account_no"], 
            order_div, 
            jongmok_code.replace("A", ""), 
            order_qty, 
            0, 
            "03", 
            ""
...
def execute():
    ...
    while True:
        ...
        # 최고, 최저 가격 설정
        # 마감 5분 전부터 매도인 경우 최고가보다 크면 매도 매수인 경우는 최저가보다 작으면 매수
        ...
        if order_div == 1:
            ...
            if (now_price > price_over or now_price < price_under):
                order_qty = int(deal_amount / (now_price * 1.3))
                # 매수
                buy_n_sell(order_div, order_qty)
                # 거래내역 리스트로 저장
            ...
            # 마감 5분전인데 최저가이면 매수후 종료
            elif run_hms > "152500":
                if now_price == low_price:
                    order_qty = int(deal_amount / (now_price * 1.3))
                    # 매수
                    buy_n_sell(order_div, order_qty)
                    # 거래내역 리스트로 저장
        ...
        # 매도
            ...
            # 최초 매수가 이후 5틱 더 내려간 경우
            if (buy_cnt < buy_max and now_price < bought_price - (5 * tick_amount)):
                buy_cnt += 1
                # 매수
                buy_n_sell(order_div, order_qty)
                # 매수금액 변경
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Kiwoom login completed!!")
        # 주식계좌
        accounts = kiwoom.GetLoginInfo("ACCNO")
        stock_account = accounts[0]
        print(stock_account)
        dict_account["account_no"] = str(stock_account)
        # 예수금
        dict_account["possesion_amount"] = get_possesion()
    except:
        print("DaeShin login Failed!!")
        run_tf = False
    # login - daeshin
    instCpCybos = win32com.client.Dispatch("CpUtil.CpCybos")
    if instCpCybos.IsConnect == 1:
        print("DaeShin login completed!!")
    else:
        print("DaeShin login Failed!!")
        run_tf = False  
    # 로그인이 모두 성공한 경우만 수행
